# Preserving TektonHub and its catalog on the 4.0.18 upgrade (DEVOPS-44544)

This document is the design of record for the release-4.0 behavior that lets one
4.0.18 codebase treat TektonHub differently on a fresh install versus an upgrade:
...
The pre-4.0.18 default deleted an existing hub on upgrade, which broke those
...
4. **InstallerSet rotation deletes the catalog ConfigMaps.** Patch `0010` keys the hub
   ...
   In a pre-4.0.18 install, 27 of the 64 catalog ConfigMaps already carry `keep`; the
...
- **`preserveExistingCatalogConfigMaps`** — in `kube-public`, selects catalog
  ...
  Findings below): a pre-4.0.18 install created the `-latest` tool-image and
...
> Shipped as patch `0022-installerset-rotation-barrier-on-upgrade.patch` (the two
> migrations of §2 ship as patch `0021`; the three-state switch as patch `0019`).
...
1. The operator restarts at v4.0.18. Both reconcilers start.
2. The TektonHub reconciler wants to rotate (version changed). Barrier:
   `PreUpgradeVersion != v4.0.18` -> requeue. The old InstallerSet, the old Deployment
...
exercised end-to-end on a live global cluster on the **release-4.10 origin** of this
change (OLM-managed operator, against `v4.10.2-pr...--44544-preserve-hux`), and again on
**release-4.6** (`v4.6.3` -> `v4.6.4`, all three scenarios green). release-4.0 carries the
**identical** mechanism (patches `0019` three-state switch + `0021` preserve/pin
migrations + `0022` InstallerSet rotation barrier), hand-adapted to release-4.0's older
upstream submodule (`v0.74.1`, operator 0.74.x) since 4.0 pins a different upstream than
4.6/4.10 — the Go changes and their semantics are unchanged, so the verification below
applies equally to the `v4.0.18` upgrade. The `v4.10.1`/`v4.10.2` version labels in the Method section below
name the origin run's baselines; the corresponding `release-4.0` transition is
`v4.0.17` -> `v4.0.18`. Every scenario carries the same mandatory acceptance: **a
`resolver: hub` pipeline still resolves and runs successfully after the upgrade** — a
green upgrade with a broken resolver is a failure.
