...
| DevOps Hub | Catalog experience for reusable Tasks and Pipelines. In current releases the runtime data source is the `artifacthub-shim` plugin, not a Tekton Operator-managed Tekton Hub runtime. |
...
3. **Component Configurations**: Specific settings for each component
   ...
   - **DevOps Hub catalog**: Provided by the separate `artifacthub-shim` plugin; configure resolver defaults under `pipeline.hub-resolver-config` when custom install locations are used.
