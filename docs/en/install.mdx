...
- `pipelines-4.10`: Locks upgrades to `v4.10.z` patch releases only.
...
- Verify that all components of the `Alauda DevOps Pipelines` Operator have been successfully installed. Log in to the cluster in a terminal and run the following command:
  ...
  **Example Output**
  ...
  If you have enabled the built-in `Tekton Hub` (not deployed by default since `v4.12.0`), also check its status with `kubectl get tektonhub`.
...
<Callouts>
  1. The channel name you want to subscribe to. `stable` is the default and installs the latest LTS version. Use `latest` to receive every release, or `pipelines-4.10` to stay on `v4.10.z` patch updates.
