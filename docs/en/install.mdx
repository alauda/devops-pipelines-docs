...
- `stable` (default, from `v4.2.5` onward): Installs the latest LTS version. Recommended for production environments.
- `latest`: Installs every available `v4.2.z` release. If you previously relied on the `latest` default, you can keep this value in your Subscription.
- `pipelines-4.2`: Locks upgrades to `v4.2.z` patch releases only.
...
<Callouts>
  1. From `v4.2.5` onward, `stable` is the default and installs the latest LTS version. Use `latest` to keep the pre-`v4.2.5` behavior (every `v4.2.z` release), or `pipelines-4.2` to stay on `v4.2.z` patch updates only.
