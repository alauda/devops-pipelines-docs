...
- `stable` (default, from `v4.6.3` onward): Installs the latest LTS version. Recommended for production environments.
- `latest`: Installs every available `v4.6.z` release. If you previously relied on the `latest` default, you can keep this value in your Subscription.
- `pipelines-4.6`: Locks upgrades to `v4.6.z` patch releases only.
...
<Callouts>
  1. From `v4.6.3` onward, `stable` is the default and installs the latest LTS version. Use `latest` to keep the pre-`v4.6.3` behavior (every `v4.6.z` release), or `pipelines-4.6` to stay on `v4.6.z` patch updates only.
