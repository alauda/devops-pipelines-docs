...
- `stable` (default, from `v4.0.17` onward): Installs the latest LTS version. Recommended for production environments.
- `latest`: Installs every available `v4.0.z` release. If you previously relied on the `latest` default, you can keep this value in your Subscription.
- `pipelines-4.0`: Locks upgrades to `v4.0.z` patch releases only.
...
<Callouts>
  1. The channel name you want to subscribe to. From `v4.0.17` onward, `stable` is the default and installs the latest LTS version. Use `latest` to keep the pre-`v4.0.17` behavior (every `v4.0.z` release), or `pipelines-4.0` to stay on `v4.0.z` patch updates only.
