...
- Download the operator package and save it to the working directory on the operation machine.

  - Downloading the Operator

    :::info 

    Alauda Devops Pipelines can be retrieved from Alauda Cloud.
  ...
  - Uploading the Operator
    ...
    2. `<operator-package>`: The name of the operator package (Example: `tektoncd-operator.latest.amd64.v4.7.0.tgz`).
    ...
    7. `--force`: Force override existing images and artifacts of registry.

    </Callouts>

    :::warning

    If you are uploading a new version to update an existing `tektoncd-operator`, the `--force` parameter is required when using **violet** to upload the `Operator`, in order to overwrite existing images and artifacts in the registry.
...
:::note
Starting from `v4.14.0`, the built-in `Tekton Hub` is no longer shipped and cannot be deployed by any preset. Tekton catalog artifacts are delivered by the platform's standalone `ArtifactHub` component instead. For catalog publishing and consumption, see <ExternalSiteLink name="artifacthub-shim" href="/catalog/index.html" children="ArtifactHub Shim Catalog" />.
:::

**Channel quick reference**

- `stable` (default): Installs the latest LTS version. Recommended for production environments.
- `latest`: Installs the latest available version, including non-LTS releases. Recommended for development and testing environments.
- `pipelines-4.14`: Locks upgrades to `v4.14.z` patch releases only. A `pipelines-x.y` channel is published for every LTS version, so `pipelines-4.10` remains available for the previous LTS line.
...
- Verify that all components of the `Alauda DevOps Pipelines` Operator have been successfully installed. Log in to the cluster in a terminal and run the following command:
  ...
  **Example Output**
  ...
  ```bash
  $ kubectl get tektonpipeline,tektontrigger,tektonchain
  ```
  ...
  **Example Output**
  ...
  The built-in `Tekton Hub` is not part of this release, so there is no `tektonhub` resource to check.
...
<Callouts>
  1. The channel name you want to subscribe to. `stable` is the default and installs the latest LTS version. Use `latest` to receive every release, or `pipelines-4.14` to stay on `v4.14.z` patch updates.
