...
- This guide uses a simplified workflow by generating the Containerfile inline within the pipeline
...
### Step 2: Set up Authentication \{#step-2-set-up-authentication}
...
### Step 3: Configure Tekton Chains \{#step-3-configure-tekton-chains}
...
### Step 4: Create a Sample Pipeline \{#step-4-create-a-sample-pipeline}
...
- `params`: The parameters for the pipeline.
  - `generate-containerfile`: The script to generate a Containerfile for building an image.
  - `image`: The target image address built.
- `tasks`: The tasks for the pipeline.
  - `generate-containerfile`: The task to generate a Containerfile for building an image.
...
- `workspaces`: The workspaces for the pipeline.
  ...
  - `registryconfig`: The workspace for distribution registry configuration.
:::
...
### Step 5: Run a Sample Pipeline \{#step-5-run-a-sample-pipeline}
...
### Step 6: Wait for the PipelineRun to be signed \{#step-6-wait-for-the-pipelinerun-to-be-signed}
...
### Step 7: Get the image from the PipelineRun \{#step-7-get-the-image-from-the-pipelinerun}
...
### Step 8: Verifying the Image and Attestation \{#step-8-verifying-the-image-and-attestation}
