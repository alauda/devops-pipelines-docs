...
<a id="installing-using-operator-crd"></a>

### Installing using TektonConfig CR \{#installing-using-tektonconfig-cr}
...
:::details \{title="Explanation of YAML fields"}
  In the TektonConfig resource, `targetNamespace` is configured under `spec`, and Tekton Results fields are configured under `spec.result`.
  ...
  - `logs_type`: The type of the logs.
    ...
    - `S3`: Store logs in S3-compatible object storage.
  - `secret_name`: The name of the secret containing S3 credentials.
  - `logs_path`: The path of the logs.
  - `logs_buffer_size`: The buffer size of the logs.
  - `logging_pvc_name`: The name of the PVC to store the logs when using `File` logs type.
