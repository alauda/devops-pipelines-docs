...
For self-signed or private-CA Git providers, configure the CA trust for the Enhancement and PAC controllers separately from the provider-side webhook verification setting. See [Configure Custom Certificates](../pac/install/configure_custom_certificates.mdx) for the complete certificate mounting examples.
          ...
          data:
            config: |
              ...
              pac-webhook-registration:
                ...
                enabled: true
                # Keep enabled in production. This controls Git provider -> PAC
                # webhook delivery only; it does not disable controller -> Git
                # provider API TLS verification.
...
| `pac-webhook-registration.webhook-target-tls-verify` | Whether GitLab or GitHub verifies the PAC webhook URL certificate when delivering events. This only affects the provider -> PAC webhook connection; it does not affect TLS verification from `tektoncd-enhancement-controller` or `pipelines-as-code-controller` to the Git provider API. | `true` |
...
If the Git provider cannot validate a self-signed PAC webhook certificate, set `webhook-target-tls-verify: false` only for non-production environments. This setting does not resolve `x509` or `certificate signed by unknown authority` errors in the Enhancement controller; mount the Git provider CA and set `SSL_CERT_FILE`/`GIT_SSL_CAINFO` as described in [Configure Custom Certificates](../pac/install/configure_custom_certificates.mdx).
...
For automatic webhook registration failures, inspect the Enhancement controller logs:
...
`x509` or `certificate signed by unknown authority` errors refer to the controller-to-Git-provider API connection. A failed webhook delivery test in GitLab or GitHub refers to the provider-to-PAC connection and should be investigated with `pac-webhook-registration.webhook-target-tls-verify` and the PAC webhook certificate.
