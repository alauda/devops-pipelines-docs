...
- [Application CI/CD Tutorials](./tutorials/index.mdx): Build a frontend and backend, save Run Templates, automate CI, and deliver an approved image by digest. Start with manual CI and add the other capabilities when you are ready.

- [How to Configure Dynamic Forms](./how_to/configure_dynamic_forms.mdx):  Configure dynamic forms for UI-based Pipeline and Task operations, enabling dropdown menus, validation rules, and dynamic data loading without writing frontend code.

- [Output Markdown to the Overview Tab (PipelineRuns & TaskRuns)](./how_to/output-markdown-to-the-overview-tab.mdx): You can configure your TaskRuns and PipelineRuns to display formatted markdown content in the Overview tab, making it easier to visualize results, summaries, or reports directly in the UI.

- <ExternalSiteLink name="artifacthub-shim" href="/catalog/task-runtime-guides.html" children="Catalog Task Runtime Guides" />: Reuse built-in catalog Task capabilities such as `run-script`, `kubectl-app-manager`, `buildah`, `merge-image`, and `skopeo-copy`.

- For more information on Pipeline configuration and usage, please refer to [How to](./how_to/index.mdx).
