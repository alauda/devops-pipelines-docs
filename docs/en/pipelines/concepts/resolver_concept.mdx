...
Catalog resources for Hub resolver are served by the standalone ArtifactHub component in current releases. For catalog configuration and authoring, see <ExternalSiteLink name="artifacthub-shim" href="/catalog/index.html" children="ArtifactHub Shim Catalog" />.
...
The Hub Resolver fetches resources from the DevOps Hub catalog. In ACP DevOps
4.13 and later, the default Hub backend is the in-cluster
`artifacthub-shim` service using Artifact Hub-compatible APIs.

#### Use Cases

- Discovering and using curated Tasks and Pipelines in air-gapped clusters
- Accessing a catalog served by `artifacthub-shim`
- Standardizing on well-tested and maintained resources

#### Principles

The Hub Resolver:
- Connects to the configured Artifact Hub API endpoint
- Retrieves the specified resource by name and version
- Supports different catalogs and versions
- Uses `catalog` for built-in Tasks and `catalog-pipelines` for built-in Pipelines
