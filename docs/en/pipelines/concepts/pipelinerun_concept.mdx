...
TaskRunSpecs allow customizing individual TaskRun configurations within a PipelineRun by specifying a list of `PipelineTaskRunSpec` which enables setting `ServiceAccountName`, Pod template, `computeResources`, and `Metadata` for each task.
...
- **computeResources**: Compute resource requirements (requests and limits) for this Task, overriding the defaults defined in the Task spec
...
**Example 5: Overriding computeResources for Specific Tasks**

Use `taskRunSpecs[].computeResources` to assign different CPU/memory quotas to individual Tasks,
overriding any defaults defined in the Task spec:
...
:::note
`computeResources` in `taskRunSpecs` is a Beta feature and requires `enable-api-fields: beta` (or `alpha`) in the Tekton feature flags ConfigMap. On the platform's default installation, `enable-api-fields` is already set to `beta`, so this feature works out of the box — you only need to adjust the flag if it has been manually set to `stable`.
:::
