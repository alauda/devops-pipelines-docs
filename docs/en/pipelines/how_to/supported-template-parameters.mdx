...
- `artifacthub-shim-extension` is installed and its template-render webhook is enabled.
...
| `detailsURL` | string | URL to the run details page. For example: `https://devops.example.com/console-acp/workspace/demo~cluster-a~dev/pipeline/pipelineRuns/detail/pr-abc123`. To adjust this value, update `extension.templateRender.detailsURLTemplate` in the `artifacthub-shim` chart values. | No |
| `startsAt` | time | Start time of the current execution. | No |
| `timeZone` | string | Global time zone configured for template rendering. The value must be the name of a valid [time zone](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones). To adjust this value, update `extension.templateRender.timeZone` in the `artifacthub-shim` chart values. | No |
...
If you want to change it, update `extension.templateRender.timeZone` in the `artifacthub-shim` chart values.
...
If `detailsURL` is not as expected, update `extension.templateRender.detailsURLTemplate` in the `artifacthub-shim` chart values.
