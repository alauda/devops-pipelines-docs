...
This page is a command reference for users who prefer the CLI workflow. For the recommended end-to-end procedure that uses plain manifests, see [Configure GitHub Repository](../guides/configure_github_repo.mdx) and [Configure GitLab Repository](../guides/configure_gitlab_repo.mdx).

## Prerequisites \{#prerequisites}

- [`tkn` CLI](https://github.com/tektoncd/cli/releases) installed
...
Download `tkn` from [Tekton CLI Releases](https://github.com/tektoncd/cli/releases), then install the `pac` plugin from [tkn pac Releases](https://github.com/tektoncd/pipelines-as-code/releases).
...
### tkn pac create repository \{#tkn-pac-create-repository}
...
For provider-specific end-to-end procedures, see [Guides](../guides/index.mdx).

### tkn pac list \{#tkn-pac-list}
...
### tkn pac repo describe \{#tkn-pac-repo-describe}
...
### tkn pac generate \{#tkn-pac-generate}
...
### tkn pac resolve \{#tkn-pac-resolve}
...
- Resolves task annotations (fetches tasks from the configured DevOps Hub catalog if needed)
...
### 2. Pipeline Generation \{#2-pipeline-generation}
...
- [Manage PAC Component](../install/manage_pac.mdx) - PAC component management
