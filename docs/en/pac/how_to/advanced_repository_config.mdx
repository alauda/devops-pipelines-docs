...
i18n:
  title:
    en: Advanced Repository Configuration
    zh: 高级仓库配置
title: Advanced Repository Configuration
---
...
- Shared configuration management using Kustomize
- Concurrency limits
...
With the global CR above, you can create individual Repository CRs with minimal configuration:
    ...
    spec:
      url: "https://gitlab.com/user/my-app.git"
      # git_provider settings are inherited from the global CR
    ```

## Managing Shared Configuration

For easier management of shared configurations, **use Kustomize** - the recommended approach for managing multiple Repository CRs with shared configuration:

**Create a base configuration** (`base/repository.yaml`):
...
**Create overlays for each repository** (`overlays/repo-1/kustomization.yaml`):
    ...
    - patch: |-
        ...
        metadata:
          name: base-repo
        spec:
          url: "https://gitlab.com/user/repo-1.git"
    ```

**Apply the configuration**:

    ```bash
    kubectl apply -k overlays/repo-1/
    ```

**Why Kustomize**:

- Native Kubernetes tool for configuration management
- Easy to maintain and version control
- Supports complex configuration scenarios
- Clear separation between base configuration and environment-specific overrides

### Alternative: Using ConfigMaps

If you need to manage shared parameter values, you can use ConfigMaps. However, note that Repository CRs cannot directly reference ConfigMap values - you need to manually copy values from ConfigMaps when creating Repository CRs, or use templating tools:

Create a ConfigMap with shared values:

    ```yaml
    apiVersion: v1
    kind: ConfigMap
    metadata:
      name: shared-pac-config
      namespace: project-pipelines
    data:
      default-concurrency-limit: "5"
      default-environment: "production"
      default-cluster-name: "shared-cluster"
    ```

Then manually reference these values when creating Repository CRs, or use templating tools:
    ...
    spec:
      ...
      # Copy values from ConfigMap manually or use templating
      concurrency_limit: 5  # From shared-pac-config
      params:
      - name: environment
        value: "production"  # From shared-pac-config
      - name: cluster-name
        value: "shared-cluster"  # From shared-pac-config
    ```

**Note**: For automated ConfigMap value injection, consider using:
- **Helm**: Use `{{ .Values.environment }}` in templates
- **Kustomize**: Use `replacements` or `patches` to inject ConfigMap values
- **CI/CD pipelines**: Use scripts to read ConfigMap and generate Repository CRs

### Best Practices for Shared Configuration

- **Use Kustomize for production**: Kustomize is the recommended approach for managing multiple Repository CRs with shared configuration
- **Global CR for simple cases**: Use the global Repository CR for simple, organization-wide defaults (Tech Preview)
- **Version control**: Store base configurations and overlays in version control
- **Documentation**: Document which settings are shared and which are repository-specific
- **Testing**: Test configurations with a few repositories before rolling out widely
- **Separation of concerns**: Keep environment-specific settings in overlays, common settings in base
...
By default, PAC fetches pipeline definitions from the branch that triggered the event. You can configure PAC to fetch pipeline definitions from a different branch.

### Configure Source Branch

Use the `pipelinesascode.tekton.dev/branch` annotation in the Repository CR:
...
**Behavior**:
- When set, PAC always fetches pipeline definitions from the specified branch
- PipelineRuns still execute with the code from the triggering branch/commit
- Useful for maintaining stable pipeline definitions while testing code changes

### Use Cases

**Stable pipeline definitions**: Keep pipeline definitions in `main` branch while testing in feature branches:
...
**Separate pipeline branch**: Maintain pipelines in a dedicated branch:

    ```yaml
    metadata:
      annotations:
        pipelinesascode.tekton.dev/branch: "pipelines"
    ```

**Default behavior** (no annotation): Fetch from the triggering branch:

    ```yaml
    metadata:
      # No branch annotation - uses triggering branch
    ```

### Using Default Branch with Settings

Alternatively, you can use the `settings.pipelinerun_provenance` field to automatically fetch pipeline definitions from the repository's default branch (as configured in the Git provider, such as `main`, `master`, or `trunk`).
...
This setting provides enhanced security by ensuring that pipeline definitions are fetched from the default branch rather than from pull request branches:
...
|--------------|---------------|----------------|----------|
| No annotation | Triggering branch | Standard | Development, testing |
| `pipelinesascode.tekton.dev/branch` | Specific branch | Medium | Stable pipeline definitions |
...
**Important**: Repository CR parameters use the PAC variable syntax `{{parameter_name}}`, not Tekton's `$(params.parameter_name)` syntax.

- **PAC variables** (`{{variable_name}}`): PAC replaces these **before** creating the PipelineRun. This includes:
...
- **Tekton parameters** (`$(params.param-name)`): Tekton resolves these **during** PipelineRun execution. These reference the PipelineRun's own `spec.params`.
...
**Production Repository**:
    ...
    spec:
      url: "https://gitlab.com/user/prod-repo.git"
      params:
      - name: environment
        value: "production"
      - name: api-url
...
**Staging Repository**:
    ...
    metadata:
      ...
      namespace: staging
    spec:
      url: "https://gitlab.com/user/staging-repo.git"
      params:
      - name: environment
        value: "staging"
      - name: api-url
...
### Source Branch Not Used

1. **Verify annotation**:

    ```bash
    kubectl get repository my-repo -n project-pipelines -o jsonpath='{.metadata.annotations.pipelinesascode\.tekton\.dev/branch}'
    ```

2. **Check PAC controller logs** for branch resolution errors

3. **Verify branch exists** in the repository
...
2. **Check parameter syntax**: Ensure you're using `{{param-name}}` syntax to reference Repository CR parameters, not `$(params.param-name)`
