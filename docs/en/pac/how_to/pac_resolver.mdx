...
4. **Queries the Hub API**:
   - **Hub URL**: Defaults to the in-cluster ArtifactHub Shim (`http://artifacthub-shim-api.artifacthub-shim-system.svc.cluster.local`); see [Custom Hub URL](#custom-hub-url) to point it elsewhere
...
### Remote Tasks from Tekton Hub \{#remote-tasks-from-tekton-hub}
...
The default `hub-url` points to the in-cluster ArtifactHub Shim
(`http://artifacthub-shim-api.artifacthub-shim-system.svc.cluster.local`, with
`hub-catalog-type: artifacthub`). Leave it unset unless you need a different Hub. To use
one, set it in the `OpenShiftPipelinesAsCode` CR — always together with
`hub-catalog-type`, because the shipped `artifacthub` value stays in place if you set only
the URL:
...
For a Tekton Hub instance instead, the catalog type has to change with it. This release does
not deploy a Tekton Hub, so there is no in-cluster address to copy — use the namespace your
own instance runs in:
...
2. **Verify Hub configuration**: read the rendered `pipelines-as-code` ConfigMap rather than the CR — `spec.settings.hub-url` is empty on a default install, because the value comes from the ConfigMap the operator ships
