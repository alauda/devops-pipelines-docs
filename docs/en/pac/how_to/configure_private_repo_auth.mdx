...
Update your Repository CR to reference the authentication secret:
    ...
    spec:
      url: "https://gitlab.com/user/private-repo.git"
      git_provider:
        type: gitlab
...
Example output (abbreviated, showing authentication section):
...
Look for any status errors or authentication issues.

### Check PAC Controller Logs

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=100 | grep -i auth  # Replace <pac-namespace> with your actual namespace (default: pipelines-as-code)
    ```

Example output (example log entries):

    ```
    {"level":"info","ts":"2024-01-01T12:00:00Z","logger":"controller","msg":"Authenticated successfully","repository":"my-repo"}
    ```

Look for authentication errors or successful access messages.
...
Example output:
    ...
    To https://gitlab.com/user/private-repo.git
...
3. Restart PAC controller pods if needed:
    ```bash
    kubectl rollout restart deployment pipelines-as-code-controller -n <pac-namespace>  # Replace <pac-namespace> with your actual namespace (default: pipelines-as-code)
    ```
