...
This guide is for **regular users** who define PAC `PipelineRun` manifests in their Git repositories.

:::

This guide explains where PAC reads pipeline files from, how to structure `PipelineRun` manifests, and how to use annotations to match Git events.
...
### Multiple Pipeline Files \{#multiple-pipeline-files}
...
## Pipeline Definition Structure \{#pipeline-definition-structure}
...
### Using Existing Pipeline \{#using-existing-pipeline}
...
### Matching a Pull Request Event to a Pipeline Run \{#matching-a-pull-request-event-to-a-pipeline-run}
...
### Matching a Push Event to a Pipeline Run \{#matching-a-push-event-to-a-pipeline-run}
...
### Excluding Path Changes \{#excluding-path-changes}
...
### Combining Path Change and Ignore \{#combining-path-change-and-ignore}
...
### CEL Expression Format \{#cel-expression-format}
...
### Available CEL Fields and Functions \{#available-cel-fields-and-functions}
...
- `event_title`: Matches the title of the event, such as the commit title for a push event, and the title of a pull or merge request for a `pull_request` event. This documentation covers GitHub and GitLab.
- `last_commit_title`: The title of the last commit in the event (Platform enhancement - not available in upstream Tekton)

#### File Change Fields \{#file-change-fields}
...
#### Match Pull Request from Specific Branch \{#match-pull-request-from-specific-branch}
...
#### Path-Based Filtering with `.pathChanged()` \{#path-based-filtering-with-pathchanged}
...
#### Match Multiple Path Patterns \{#match-multiple-path-patterns}
...
#### Event Title Matching \{#event-title-matching}
...
### Complex CEL Expression Example \{#complex-cel-expression-example}
...
#### Matching PipelineRun by Branch with Regex \{#matching-pipelinerun-by-branch-with-regex}
...
#### Matching PipelineRun by File Changes with Files Property \{#matching-pipelinerun-by-file-changes-with-files-property}
...
#### Filtering PipelineRuns to Exclude Non-Code Changes \{#filtering-pipelineruns-to-exclude-non-code-changes}
...
#### Matching PipelineRun to Event Title \{#matching-pipelinerun-to-event-title}
...
#### Matching PipelineRun on Body Payload \{#matching-pipelinerun-on-body-payload}
...
#### Matching PipelineRun to Request Header \{#matching-pipelinerun-to-request-header}
...
### Configure Cancellation \{#configure-cancellation}
...
#### Using Git Authentication Secret \{#using-git-authentication-secret}
...
**Note**: The `{{ git_auth_secret }}` variable is only available for private repositories. PAC automatically creates this secret when the `Repository` references a Git access token; see [Create a Git Secret for PAC](./create_git_secret.mdx). The secret name format is `pac-gitauth-<repository-name>-<hash>`.
...
#### Remote Task Annotations \{#remote-task-annotations}
...
#### Tekton Hub Tasks \{#tekton-hub-tasks}
...
#### Remote URL Tasks \{#remote-url-tasks}
...
### Simple Build Pipeline \{#simple-build-pipeline}
...
### Test Pipeline for PRs \{#test-pipeline-for-prs}
