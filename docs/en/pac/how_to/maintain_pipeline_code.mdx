...
i18n:
  title:
    en: Maintain Pipeline Code
    zh: 维护流水线代码
title: Maintain Pipeline Code
---
...
- `last_commit_title`: The title of the last commit in the event
...
#### Additional CEL Fields

Beyond the basic fields, CEL expressions support additional fields for advanced filtering:

| Field | Description | Example |
|-------|-------------|---------|
| `files.all` | All changed files | `files.all.exists(x, x.matches('pattern'))` |
| `files.added` | Files that were added | `files.added.exists(x, x.matches('pattern'))` |
| `files.deleted` | Files that were deleted | `files.deleted.exists(x, x.matches('pattern'))` |
| `files.modified` | Files that were modified | `files.modified.exists(x, x.matches('pattern'))` |
| `files.renamed` | Files that were renamed | `files.renamed.exists(x, x.matches('pattern'))` |
| `body` | Full payload body from Git provider | `body.pull_request.base.ref == "main"` |
| `headers` | HTTP headers from Git provider | `headers['x-github-event'] == "pull_request"` |

**Note**: The `files.*` properties are available for both push and pull_request events. For pull requests, every file belonging to the pull request will be listed. The `body` and `headers` properties provide access to the raw payload data, allowing for very specific filtering based on Git provider-specific information.
...
| `{{repo_url}}` | The repository full URL | `https://gitlab.com/user/repo.git` | All events |
...
Reference tasks from remote URLs:
