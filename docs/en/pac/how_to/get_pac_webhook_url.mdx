...
If `EXTERNAL_IP` is empty, configure the cluster LoadBalancer provider first. On ACP bare-metal clusters, use **Alauda Container Platform Load Balancer for MetalLB**. Reference: <ExternalSiteLink name="acp" href="/networking/functions/configure_metallb" children="Configure MetalLB" />.
