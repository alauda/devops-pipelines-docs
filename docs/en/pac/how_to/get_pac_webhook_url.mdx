...
Choose the section that matches how PAC was exposed: Gateway API, Ingress, or NodePort. Examples use the default PAC namespace `tekton-pipelines`; replace it in the commands if your `TektonConfig` uses a different `targetNamespace`.
...
Update the `pipelinesAsCode` options in `TektonConfig`, under `platforms.kubernetes`. The operator rejects the same block under `platforms.openshift` with a validation error:
...
Do not put this block on the derived `OpenShiftPipelinesAsCode` CR instead: the operator copies `options` back over from `TektonConfig` on the next reconcile, so the URL is lost. The one exception is a cluster where `TektonConfig` does not manage PAC at all — `spec.profile` is `basic` or `lite` — in which case the same `options` block goes on that CR under `spec.options`. See [How the Operator Manages PAC](../install/manage_pac.mdx#how-the-operator-manages-pac).
