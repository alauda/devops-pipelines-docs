...
### How to Get PAC Controller URL \{#how-to-get-pac-controller-url}
...
| `custom-console-url-namespace-vars` | Extra template variables resolved from the PipelineRun namespace labels/annotations (e.g. `{{ project }}`, `{{ cluster }}`), usable in the `custom-console-url*` templates above | `` (empty) |
...
- The `custom-console-url-namespace-vars` declares extra variables (such as `{{ project }}` and `{{ cluster }}`) resolved from the PipelineRun namespace labels/annotations. See [Configure Custom Console Links](#configure-custom-console-links) for details.
...
To integrate PAC with your custom console, configure custom console links. This makes the pipeline status links in Git providers (GitLab, GitHub, etc.) point to your custom console, with `{{ project }}` and `{{ cluster }}` resolved automatically from the PipelineRun namespace.

##### Quick Start

Apply the configuration below and replace `console.example.com` with your platform host — everything else works as-is:
    ...
    spec:
      ...
      settings:
        ...
        # URL template for the TaskRun log page
        ...
        # URL template for the namespace pipeline list page
      ...
      options:
        configMaps:
          pipelines-as-code:
            data:
              # Render an unset template variable as empty instead of keeping {{ var }} in the URL
              replace-empty-template-vars-with-empty: "true"
    ```

##### How the Variables Are Resolved

PAC replaces the following variables in the URL templates:

- `{{ namespace }}`: The namespace where the PipelineRun runs
- `{{ pr }}`: The PipelineRun name
- `{{ task }}`: The Task name in the PipelineRun
- `{{ project }}` and `{{ cluster }}`: Resolved from the PipelineRun namespace labels declared in `custom-console-url-namespace-vars`. By default `{{ project }}` comes from the `cpaas.io/project` label and `{{ cluster }}` from the `cpaas.io/cluster` label, which the platform sets on each namespace. This avoids hard-coding a cluster name per environment.

`custom-console-url-namespace-vars` is a comma-separated list; each entry is `name=label:<key>` or `name=annotation:<key>`, with an optional `|<default>` used when the label or annotation is missing or empty. You can change the label keys, read from annotations, or declare additional variables to match your platform. For example, to fall back to `unknown` when the project label is absent:
...
:::info
Namespace labels and annotations are cached for up to 30 minutes, so a label change may take a few minutes to appear in the generated links.
:::

##### Example: Generated Links

When PAC creates a PipelineRun named `my-app-build-abc123` with a task `build-task` in namespace `my-app` (labeled `cpaas.io/project=team-a`, `cpaas.io/cluster=prod`):
