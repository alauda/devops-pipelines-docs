...
i18n:
  title:
    en: Configure Custom Certificates
    zh: 配置自定义证书
title: Configure Custom Certificates
---
...
Verify the certificate is mounted in the PAC controller pod:
    ...
    kubectl exec -n ${PAC_NAMESPACE} \
      $(kubectl get pod -n ${PAC_NAMESPACE} -l app=pipelines-as-code-controller -o jsonpath='{.items[0].metadata.name}') \
      -- ls -la /etc/ssl/certs/git-ca.crt
    ```

Example output:

    ```
    -rw-r--r-- 1 root root 1234 Jan  1 12:00 /etc/ssl/certs/git-ca.crt
    ```

You should see the certificate file listed.
...
Verify the environment variables are set:
    ...
    kubectl exec -n ${PAC_NAMESPACE} \
      $(kubectl get pod -n ${PAC_NAMESPACE} -l app=pipelines-as-code-controller -o jsonpath='{.items[0].metadata.name}') \
      -- env | grep -E "GIT_SSL_CAINFO|SSL_CERT_FILE"
    ```

Example output:

    ```
    GIT_SSL_CAINFO=/etc/ssl/certs/git-ca.crt
    SSL_CERT_FILE=/etc/ssl/certs/git-ca.crt
    ```

### Test Git Operations

Test that Git can access your Git hosting service:
    ...
    kubectl exec -n ${PAC_NAMESPACE} \
      $(kubectl get pod -n ${PAC_NAMESPACE} -l app=pipelines-as-code-controller -o jsonpath='{.items[0].metadata.name}') \
      -- git config --global http.sslCAInfo /etc/ssl/certs/git-ca.crt
    ```
...
Check for SSL/certificate errors in the PAC controller logs:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=100 | grep -i ssl  # Replace <pac-namespace> with your actual namespace (default: tekton-pipelines)
    ```

Example output (if configured correctly, should be empty or show no SSL errors):

    ```
    (no output or no SSL errors)
    ```

If configured correctly, you should not see SSL certificate errors.
...
1. Check CR status:
    ```bash
    kubectl get openshiftpipelinesascodes -n <pac-namespace>  # Replace <pac-namespace> with your actual namespace (default: tekton-pipelines)
    ```
...
2. Check Operator logs:
    ```bash
    kubectl logs -n <pac-namespace> -l app.kubernetes.io/name=operator --tail=100  # Replace <pac-namespace> with your actual namespace (default: tekton-pipelines)
    ```
...
- [Configure Authentication for Private Repositories](../how_to/configure_private_repo_auth.mdx) - Configure authentication for private repositories
- [Manage PAC Component](./manage_pac.mdx) - Learn about managing PAC deployment
- [Configure GitLab Repository](../how_to/configure_gitlab_repo.mdx) - GitLab-specific configuration guide
