---
weight: 10
---

# Common Issues

:::info For All Users

This troubleshooting guide covers issues for both **administrators** (component deployment, configuration) and **regular users** (repository setup, pipeline execution).

**Important**: Throughout this document, we use two different namespaces:
- **`<pac-namespace>`**: The namespace where PAC components (controller, watcher, webhook) are deployed. Default is `tekton-pipelines`, but can be customized via `targetNamespace` in the `TektonConfig` CR.
- **`<namespace>`**: The namespace where PipelineRuns are created. This is specified when creating the Repository CR and can be any namespace in your cluster.

Replace these placeholders with your actual namespace names.

:::

This document provides troubleshooting steps for common issues encountered when using Pipelines-as-Code (PAC).

## Before You Start \{#before-you-start}

Before troubleshooting specific issues, verify these basics:

### 1. Check PAC Components Are Running \{#1-check-pac-components-are-running}

Verify all PAC pods are running (replace `<pac-namespace>` with your PAC namespace, default is `tekton-pipelines`):

```bash
kubectl get pods -n <pac-namespace> | grep pipelines-as-code
```

Example output (all pods should be `Running`):

```
NAME                                          READY   STATUS    RESTARTS   AGE
pipelines-as-code-controller-769bf5c88c-xxxxx  1/1     Running   0          113d
pipelines-as-code-watcher-546d6cd86d-xxxxx     1/1     Running   15         113d
pipelines-as-code-webhook-5fcd94db5b-xxxxx     1/1     Running   0          113d
```

### 2. Determine Your Namespaces \{#2-determine-your-namespaces}

**Get PAC namespace** (where PAC components are deployed):

```bash
kubectl get tektonconfig config -o jsonpath='{.spec.targetNamespace}'
```

If the command fails or returns empty, use the default: `tekton-pipelines`

**Get Pipeline namespace** (where PipelineRuns are created):

```bash
kubectl get repository -A
```

This shows all Repository CRs and their namespaces. The namespace column shows where PipelineRuns will be created.

### 3. Get Resource Names \{#3-get-resource-names}

**Get pod names**:

```bash
kubectl get pods -n <pac-namespace> | grep pipelines-as-code
```

**Get Repository CR names**:

```bash
kubectl get repository -n <namespace>
```

**Get PipelineRun names**:

```bash
kubectl get pipelinerun -n <namespace>
```

### 4. Common Placeholders Used in This Document \{#4-common-placeholders-used-in-this-document}

- `<pac-namespace>`: PAC deployment namespace (default: `tekton-pipelines`)
- `<namespace>`: PipelineRun namespace (varies by repository)
- `<repo-name>`: Repository CR name (use `kubectl get repository -n <namespace>` to list)
- `<pod-name>`: Pod name (use `kubectl get pods -n <pac-namespace>` to list)
- `<name>`: PipelineRun name (use `kubectl get pipelinerun -n <namespace>` to list)
- `<gitlab-secret>`: GitLab token secret name (check Repository CR spec)

## PAC Component Not Deploying \{#pac-component-not-deploying}

### Symptom \{#symptom}

`OpenShiftPipelinesAsCode` CR shows `Ready: False`, pods are not starting, or that CR does not exist at all.

### Troubleshooting Steps \{#troubleshooting-steps}

1. **Check that `TektonConfig` asks for PAC**. The operator derives the `OpenShiftPipelinesAsCode` CR from `TektonConfig`, and only when the profile is `all` and PAC is enabled. If the CR is missing entirely, start here:

    ```bash
    kubectl get tektonconfig config \
      -o jsonpath='profile={.spec.profile} enable={.spec.platforms.kubernetes.pipelinesAsCode.enable}{"\n"}'
    ```

    Expected output:

    ```text
    profile=all enable=true
    ```

    A profile of `basic` or `lite` means the operator never reaches its PAC logic — it neither creates nor deletes the CR.

    An empty `enable` means `spec.platforms.kubernetes.pipelinesAsCode` is absent altogether. The defaulting webhook fills that block in with `enable: true`, but only when something writes `TektonConfig`, so the block stays absent on a cluster whose `TektonConfig` has not been written since the operator was upgraded. On `profile: all` — the only profile that reaches the PAC logic at all — the reconciler reads that absent block as PAC being disabled and deletes the derived CR. Writing the block explicitly, as shown in [Deploy PAC Component](../install/manage_pac.mdx#deploy-pac-component), both fixes the immediate problem and keeps the intent visible in the CR.

2. **Check CR Status**:

    ```bash
    kubectl get openshiftpipelinesascodes.operator.tekton.dev pipelines-as-code -o yaml
    ```

Example output (abbreviated):

    ```yaml
    apiVersion: operator.tekton.dev/v1alpha1
    kind: OpenShiftPipelinesAsCode
    metadata:
      name: pipelines-as-code
    status:
      conditions:
      - status: "False"
        type: Ready
        reason: DeploymentFailed
    ```

3. **Check CR Events**:

    ```bash
    kubectl describe openshiftpipelinesascodes.operator.tekton.dev pipelines-as-code
    ```

Example output (abbreviated):

    ```
    Name:         pipelines-as-code
    Status:       False
    Events:
    Type     Reason           Age   From              Message
    ----     ------           ----  ----              -------
    Warning  DeploymentFailed 5m    tekton-operator   Failed to deploy PAC component
    ```

4. **Check TektonInstallerSet**:

    ```bash
    kubectl get tektoninstallersets | grep openshiftpipelinesascode
    kubectl describe tektoninstallerset <name>
    ```

   **Note**: `TektonInstallerSet` is a cluster-scoped resource. This is a read-only check for troubleshooting. Do not modify or delete it directly. If issues persist, troubleshoot through `TektonConfig` and the derived `OpenShiftPipelinesAsCode` CR.

Example output:

    ```
    NAME                                             AGE
    openshiftpipelinesascode-main-deployment-4bzwx   153d
    openshiftpipelinesascode-main-static-s7qn2       153d
    openshiftpipelinesascode-post-cdwjw              153d
    ```

5. **Check Operator Logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app=tekton-operator --tail=100
    ```

Example output (example log entries):

    ```
    {"level":"error","ts":"2024-01-01T12:00:00Z","logger":"operator","msg":"Failed to deploy PAC","error":"..."}
    ```

### Common Causes \{#common-causes}

- **PAC not requested in `TektonConfig`**: the profile is not `all`, or the profile is `all` but `spec.platforms.kubernetes.pipelinesAsCode` is missing or has `enable: false`. This is the usual reason for the CR being absent rather than unready. The two cases differ: on `basic` or `lite` the operator never creates the CR and never touches one that already exists, while on `all` a missing block or `enable: false` makes it delete the CR. The missing-block case is easy to hit after an in-place upgrade, because the block is only filled in when something writes `TektonConfig`
- **Operator not running**: Check Tekton Operator status
- **Namespace issues**: Verify `targetNamespace` exists
- **Resource conflicts**: Check for existing resources

### Solution \{#solution}

1. Set `spec.profile: all` and `spec.platforms.kubernetes.pipelinesAsCode.enable: true` in `TektonConfig`
2. Ensure Tekton Operator is running
3. Check namespace exists (replace `<pac-namespace>` with your actual PAC namespace): `kubectl get namespace <pac-namespace>`
4. If the derived CR is stuck rather than missing, delete it — on `profile: all`, with PAC still enabled in `TektonConfig`, the operator recreates it on the next reconcile. Do not do this on `basic` or `lite`: nothing recreates the CR there, so deleting it removes PAC permanently

## PAC Pods Not Starting \{#pac-pods-not-starting}

### Symptom \{#symptom-2}

PAC pods are in `Pending` or `CrashLoopBackOff` state.

### Troubleshooting Steps \{#troubleshooting-steps-2}

1. **Check Pod Status** (replace `<pac-namespace>` with your PAC namespace):

    ```bash
    kubectl get pods -n <pac-namespace> | grep pipelines-as-code
    ```

Example output:

    ```
    NAME                                          READY   STATUS             RESTARTS   AGE
    pipelines-as-code-controller-769bf5c88c-xxxxx  0/1     CrashLoopBackOff   5          113d
    pipelines-as-code-watcher-546d6cd86d-xxxxx    0/1     Pending            0          113d
    ```

2. **Check Pod Logs**:

    First, get the pod name (replace `<pac-namespace>` with your PAC namespace):

    ```bash
    kubectl get pods -n <pac-namespace> | grep pipelines-as-code
    ```

    Then check logs (replace `<pod-name>` with the actual pod name):

    ```bash
    kubectl logs -n <pac-namespace> <pod-name>
    ```

Example output (example error):

    ```
    Error: failed to start controller: cannot connect to database
    ```

3. **Check Pod Events**:

    Get the pod name first, then describe it (replace `<pod-name>` with the actual pod name):

    ```bash
    kubectl describe pod <pod-name> -n <pac-namespace>
    ```

Example output (abbreviated):

    ```
    Name:         pipelines-as-code-controller-xxxxx
    Status:       CrashLoopBackOff
    Events:
    Type     Reason     Age   From               Message
    ----     ------     ----  ----               -------
    Warning  Failed     5m    kubelet            Error: ImagePullBackOff
    ```

4. **Check Resource Limits**:

    ```bash
    kubectl get pod <pod-name> -n <pac-namespace> -o yaml | grep -A 5 resources
    ```

Example output:

    ```yaml
    resources:
      limits:
        cpu: 500m
        memory: 512Mi
      requests:
        cpu: 100m
        memory: 128Mi
    ```

## Automatic Webhook Registration Fails \{#automatic-webhook-registration-fails}

### Symptom \{#symptom-automatic-webhook-registration-fails}

The GitLab/GitHub webhook is not created after applying a `Repository`, or the Repository has:

```yaml
metadata:
  annotations:
    pac.tekton.alauda.io/webhook-registration-status: Failed
```

### Troubleshooting Steps \{#troubleshooting-steps-automatic-webhook-registration-fails}

1. **Check Repository annotations and events**:

    ```bash
    kubectl describe repository <repo-name> -n <namespace>
    kubectl get repository <repo-name> -n <namespace> \
      -o jsonpath='{.metadata.annotations.pac\.tekton\.alauda\.io/webhook-registration-reason}{"\n"}'
    kubectl get repository <repo-name> -n <namespace> \
      -o jsonpath='{.metadata.annotations.pac\.tekton\.alauda\.io/webhook-registration-message}{"\n"}'
    ```

2. **Check webhook registration controller logs**:

    ```bash
    kubectl logs -n <pac-namespace> \
      -l app=tektoncd-enhancement-controller \
      --tail=100
    ```

3. **Verify the PAC webhook URL source**:

    ```bash
    kubectl get configmap pipelines-as-code-info -n <pac-namespace> \
      -o jsonpath='{.data.controller-url}{"\n"}'
    ```

    If it is empty, configure the PAC public URL and record it in `pipelines-as-code-info.data.controller-url`. See [Get the PAC Webhook URL](../how_to/get_pac_webhook_url.mdx#record-the-webhook-url).

4. **Verify token and webhook Secret references**:

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o yaml
    kubectl get secret <secret-name> -n <namespace> -o json | jq '.data | keys'
    ```

5. **Check centralized credential Secret labels** when you use a shared webhook management token:

    ```bash
    kubectl get secret -n <controller-namespace> \
      -l pac.tekton.alauda.io/provider-credential=true \
      --show-labels
    ```

### Common Causes \{#common-causes-automatic-webhook-registration-fails}

| Reason annotation | Meaning | Fix |
|---|---|---|
| `WebhookURLUnavailable` | The controller cannot find the public PAC webhook URL. | Populate `pipelines-as-code-info.data.controller-url` through `TektonConfig`. |
| `MissingWebhookSecret` | The effective `spec.git_provider.webhook_secret` is missing or points to a missing/empty key. | Add `webhook_secret.name` and key `webhook.secret` in the Repository namespace, or inherit it from the PAC global Repository. Inherited Secrets are read from the global Repository namespace. |
| `MissingCredential` | No usable provider token was found. | Add `spec.git_provider.secret` in the Repository namespace, inherit it from the PAC global Repository, or add one matching centralized Secret in the webhook registration controller namespace. Inherited Secrets are read from the global Repository namespace. |
| `AmbiguousCredential` | More than one centralized Secret matches the same provider type and URL. | Keep exactly one matching Secret, or adjust `pac.tekton.alauda.io/provider-url`. |
| `ProviderAPIError` | GitLab/GitHub API returned an error. | Check token permission, `git_provider.url`, provider reachability, and CA trust. For centralized webhook registration, GitHub needs `admin:repo_hook` or `Webhooks` write permission, and GitLab needs `api` scope with administrator or Maintainer/Owner project access. |
| `NoGitProvider` | The Repository has no local `spec.git_provider`, or the effective provider type is empty and cannot be inferred. | Add a local `spec.git_provider` block for Webhook mode. Set `type` to `github` or `gitlab`, inherit it from the PAC global Repository, or provide an unambiguous public GitHub/GitLab URL or `/api/v3` or `/api/v4` API URL. GitHub App mode does not use automatic webhook registration. |
| `UnsupportedProvider` | The controller only supports GitLab and GitHub Webhook mode. | Use manual registration or another supported integration mode. |

TLS errors depend on which component initiated the connection:

- `x509`, `certificate signed by unknown authority`, or similar errors in `tektoncd-enhancement-controller` logs mean that the controller cannot trust the Git provider API certificate. Mount the CA bundle and set `SSL_CERT_FILE`/`GIT_SSL_CAINFO`; see [Configure Custom Certificates](../install/configure_custom_certificates.mdx).
- A GitLab/GitHub webhook delivery test that fails because the PAC endpoint certificate cannot be verified is a provider-to-PAC issue. Check `pac-webhook-registration.webhook-target-tls-verify` in [Configure Enhancement Features via TektonConfig](../../configure/configure-tektoncd-enhancement-configmap.mdx), or install a CA trusted by the Git provider.
- `pipelines-as-code-controller` errors while fetching repository content or reporting status are a separate PAC-controller-to-provider trust issue and require the PAC controller CA configuration in [Configure Custom Certificates](../install/configure_custom_certificates.mdx).

### Notes \{#notes-automatic-webhook-registration}

- Repository-local token is tried first. If it returns `401` or `403`, the controller retries with a matching centralized Secret.
- Centralized credential Secrets are read from the webhook registration controller namespace; the controller does not search each Repository namespace for centralized credentials.
- Direct Repository Secrets do not need labels. The controller watches only the Secret namespace/name pairs referenced by the effective Repository spec and reconciles related Repositories when those Secrets change.
- Keep Repository-local tokens scoped for normal PAC access. Store webhook management permissions in a centralized credential when automatic webhook registration is enabled.
- PAC global Repository `git_provider` settings only apply to Repositories that have a local `spec.git_provider` block. A `spec.url`-only Repository is skipped by automatic webhook registration.
- The annotation `pac.tekton.alauda.io/webhook-managed-ref` stores the provider hook ID. When the PAC webhook URL changes but the provider and repository identity stay the same, the controller updates the recorded hook by ID instead of creating a duplicate.
- Updating `spec.url` registers a webhook for the new repository but does not delete the webhook from the old repository. Delete the old provider webhook manually if it is no longer needed.
- Deleting the Repository attempts best-effort cleanup of the recorded managed webhook. Cleanup failures are logged and emitted as events, but they do not block Repository finalizer removal. Check the provider-side webhook manually if credentials were removed or the provider API was unreachable.
- Disabling automatic registration globally, or adding `pac.tekton.alauda.io/webhook-registration-disabled: "true"` to a Repository, removes recorded managed webhooks before the controller marks the Repository inactive. If cleanup fails because credentials are missing or provider access is unavailable, the controller keeps the managed annotations and finalizer and retries after the credential or provider access is fixed.
- Add label `pac.tekton.alauda.io/webhook-registration-disabled: "true"` to a Repository to skip automatic webhook registration for that Repository.

For the controller logs, use:

```bash
kubectl logs -n <pac-namespace> \
  -l app=tektoncd-enhancement-controller \
  --tail=100 | grep -Ei 'x509|certificate|tls|webhook|ProviderAPIError'
```

## Webhook Not Receiving Events \{#webhook-not-receiving-events}

### Symptom \{#symptom-3}

GitLab webhook events are not reaching PAC, pipelines not triggering.

### Troubleshooting Steps \{#troubleshooting-steps-3}

1. **Check Webhook Configuration in GitLab**:

   - Go to GitLab project → Settings → Webhooks
   - Verify webhook URL is correct
   - Check webhook secret matches Repository CR

2. **Check PAC Controller Logs** (replace `<pac-namespace>` with your PAC namespace):

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=100
    ```

Example output (example log entries):

    ```
    {"level":"info","ts":"2024-01-01T12:00:00Z","logger":"controller","msg":"Received webhook event"}
    {"level":"error","ts":"2024-01-01T12:00:01Z","logger":"controller","msg":"Webhook validation failed","error":"invalid signature"}
    ```

3. **Test Webhook from GitLab**:

   - Go to GitLab project → Settings → Webhooks
   - Click "Test" → "Push events"
   - Check webhook response

4. **Verify Controller URL is Accessible**:

    ```bash
    # If using Gateway API
    curl -i http://pac.example.com/
    # If DNS is not ready yet, resolve the host to the Envoy Gateway external IP:
    # curl -i --resolve "pac.example.com:80:<gateway-external-ip>" http://pac.example.com/
    ```

Example output:

    ```
    HTTP/1.1 200 OK
    Content-Type: text/plain
    ```

    ```bash
    # If using Ingress
    curl -i http://pac.example.com/
    ```

Example output:

    ```
    HTTP/1.1 200 OK
    Content-Type: text/plain
    ```

    ```bash
    # If using NodePort
    curl -i http://<node-ip>:<node-port>/
    ```

Example output:

    ```
    HTTP/1.1 200 OK
    Content-Type: text/plain
    ```

5. **Check Repository CR**:

    First, list all Repository CRs to find the name (replace `<namespace>` with your Pipeline namespace):

    ```bash
    kubectl get repository -n <namespace>
    ```

    Then check the specific Repository CR (replace `<repo-name>` with the actual repository name):

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o yaml
    ```

Example output (abbreviated):

    ```yaml
    apiVersion: pipelinesascode.tekton.dev/v1alpha1
    kind: Repository
    metadata:
      name: my-repo
    spec:
      git_provider:
        type: gitlab
        webhook_secret:
          name: webhook-secret
          key: secret
    ```

### Common Causes \{#common-causes-2}

- **Controller URL not accessible**: Firewall or network issues
- **Webhook secret mismatch**: Secret in GitLab doesn't match Repository CR
- **Webhook URL incorrect**: Wrong controller URL configured
- **Gateway/Ingress/Service not configured**: Controller not exposed

### Solution \{#solution-2}

1. Verify controller URL is accessible from GitLab servers
2. Ensure webhook secret matches in both GitLab and Repository CR
3. Check Gateway/Ingress/Service configuration
4. Test webhook manually using `curl`

### Verification \{#verification}

After applying solutions, verify the fix:

1. **Test webhook from GitLab**:
   - Go to GitLab project → Settings → Webhooks
   - Click "Test" → "Push events"
   - Check that webhook returns `200 OK` response

2. **Check PAC Controller logs** for webhook events:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=50 | grep -i webhook
    ```

    You should see log entries indicating webhook events are being received.

## Pipeline Not Triggering \{#pipeline-not-triggering}

### Symptom \{#symptom-4}

Pipelines are not being created when Git events occur.

### Troubleshooting Steps \{#troubleshooting-steps-4}

1. **Check Pipeline Annotations**:

    ```bash
    cat .tekton/pipelinerun.yaml | grep pipelinesascode.tekton.dev
    ```

Example output:

    ```
    pipelinesascode.tekton.dev/on-target-branch: "[refs/heads/main]"
    pipelinesascode.tekton.dev/on-event: "[push]"
    ```

2. **Verify Branch Names Match**:

    ```bash
    git branch --show-current
    git log --oneline -1
    ```

Example output:

    ```
    main
    abc1234 Test commit
    ```

3. **Check PAC Controller Logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=100 | grep -i trigger
    ```

Example output (example log entries):

    ```
    {"level":"info","ts":"2024-01-01T12:00:00Z","logger":"controller","msg":"Processing trigger","branch":"main","event":"push"}
    {"level":"warn","ts":"2024-01-01T12:00:01Z","logger":"controller","msg":"Branch mismatch","expected":"main","actual":"develop"}
    ```

4. **Check Repository CR Status**:

    First, list Repository CRs to find the name:

    ```bash
    kubectl get repository -n <namespace>
    ```

    Then check the Repository CR (replace `<repo-name>` with the actual repository name):

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o yaml
    kubectl describe repository <repo-name> -n <namespace>
    ```

Example output (abbreviated):

    ```
    Name:         my-repo
    Namespace:    project-pipelines
    Status:       Ready
    Events:
    Type    Reason   Age   From     Message
    ----    ------   ----  ----     -------
    Normal  Ready    5m    pac      Repository validated
    ```

5. **Verify Pipeline File Exists**:

    ```bash
    git ls-files .tekton/
    ```

Example output:

    ```
    .tekton/pipelinerun.yaml
    ```

### Common Causes \{#common-causes-3}

- **Annotation syntax error**: Incorrect annotation format
- **Branch name mismatch**: Branch pattern doesn't match actual branch
- **Pipeline file not found**: File not in expected location
- **Path filtering**: Files changed don't match path filter
- **Repository CR not found**: No matching Repository CR for the repository

### Solution \{#solution-3}

1. Verify annotation syntax: Use `pipelinesascode.tekton.dev/on-target-branch: "[refs/heads/main]"` and `pipelinesascode.tekton.dev/on-event: "[push]"` instead of the deprecated `on-push` annotation
2. Check branch names match exactly (case-sensitive)
3. Ensure `.tekton/pipelinerun.yaml` exists in repository
4. Review path filtering if configured
5. Verify Repository CR exists and matches repository URL

### Verification \{#verification-2}

After applying solutions, verify the fix:

1. **Trigger a test event**:
   - Push a commit to the branch specified in your pipeline annotation
   - Or create a Merge Request if testing MR triggers

2. **Check if PipelineRun is created**:

    ```bash
    kubectl get pipelinerun -n <namespace> --sort-by=.metadata.creationTimestamp | tail -5
    ```

    You should see a new PipelineRun created after the Git event.

3. **Check PAC Controller logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=50 | grep -i trigger
    ```

    Look for log entries indicating the event was processed.

## PipelineRuns Not Created \{#pipelineruns-not-created}

### Symptom \{#symptom-5}

Webhook events are received but PipelineRuns are not created.

### Troubleshooting Steps \{#troubleshooting-steps-5}

1. **Check PAC Controller Logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=200
    ```

Example output (example error):

    ```
    {"level":"error","ts":"2024-01-01T12:00:00Z","logger":"controller","msg":"Failed to create PipelineRun","error":"namespaces \"project-pipelines\" not found"}
    ```

2. **Check Repository CR**:

    First, list Repository CRs to find the name:

    ```bash
    kubectl get repository -n <namespace>
    ```

    Then check the Repository CR (replace `<repo-name>` with the actual repository name):

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o yaml
    kubectl describe repository <repo-name> -n <namespace>
    ```

Example output (abbreviated):

    ```yaml
    apiVersion: pipelinesascode.tekton.dev/v1alpha1
    kind: Repository
    metadata:
      name: my-repo
    spec:
      git_provider:
        type: gitlab
      # ......
    ```

3. **Verify Namespace Exists**:

    ```bash
    kubectl get namespace <namespace>
    ```

Example output:

    ```
    NAME                STATUS   AGE
    project-pipelines   Active   10m
    ```

If namespace doesn't exist, you'll see:

    ```
    Error from server (NotFound): namespaces "project-pipelines" not found
    ```

4. **Check RBAC Permissions**:

    ```bash
    kubectl auth can-i create pipelineruns --namespace <namespace>
    ```

Example output:

    ```
    yes
    ```

5. **Check GitLab Token**:

    First, find the secret name from Repository CR:

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o jsonpath='{.spec.git_provider.secret.name}'
    ```

    Then check the secret (replace `<gitlab-secret>` with the actual secret name):

    ```bash
    kubectl get secret <gitlab-secret> -n <namespace> -o yaml
    ```

Example output (abbreviated, token is base64 encoded):

    ```yaml
    apiVersion: v1
    kind: Secret
    metadata:
      name: gitlab-secret
    data:
      token: Z2xwYXQt...
    ```

### Common Causes \{#common-causes-4}

- **Namespace doesn't exist**: Target namespace not created
- **RBAC permissions**: PAC ServiceAccount lacks permissions
- **GitLab token invalid**: Token expired or incorrect
- **Pipeline definition error**: Invalid YAML or Tekton syntax
- **Repository access**: Cannot access Git repository

### Solution \{#solution-4}

1. Create namespace if missing: 

    ```bash
    kubectl create namespace <namespace>
    ```

Example output:

    ```
    namespace/project-pipelines created
    ```

2. **Verify RBAC**: Check ServiceAccount and RoleBindings

    Check what ServiceAccount PAC uses:

    ```bash
    kubectl get pod -n <pac-namespace> -l app=pipelines-as-code-controller -o jsonpath='{.items[0].spec.serviceAccountName}'
    ```

    Check if ServiceAccount has required permissions:

    ```bash
    kubectl auth can-i create pipelineruns --namespace <namespace> --as=system:serviceaccount:<pac-namespace>:<service-account-name>
    ```

3. Update GitLab token if expired

4. Validate pipeline YAML syntax

5. Test GitLab token: 

    ```bash
    curl --header "PRIVATE-TOKEN: <token>" "https://gitlab.com/api/v4/user"
    ```

Example output (if token is valid):

    ```json
    {
      "id": 1,
      "username": "user",
      "email": "user@example.com"
    }
    ```

If token is invalid:

    ```
    {"message":"401 Unauthorized"}
    ```

### Verification \{#verification-3}

After applying solutions, verify the fix:

1. **Trigger a Git event** (push or create MR) and check if PipelineRun is created:

    ```bash
    kubectl get pipelinerun -n <namespace> --sort-by=.metadata.creationTimestamp | tail -1
    ```

2. **Check PipelineRun status**:

    ```bash
    kubectl describe pipelinerun <name> -n <namespace>
    ```

    The PipelineRun should be created and start executing.

## Tasks Not Found \{#tasks-not-found}

### Symptom \{#symptom-6}

PipelineRun fails with "task not found" error.

### Troubleshooting Steps \{#troubleshooting-steps-6}

1. **Check PipelineRun Status**:

    First, list PipelineRuns to find the name:

    ```bash
    kubectl get pipelinerun -n <namespace>
    ```

    Then check the PipelineRun (replace `<name>` with the actual PipelineRun name):

    ```bash
    kubectl get pipelinerun <name> -n <namespace> -o yaml
    kubectl describe pipelinerun <name> -n <namespace>
    ```

Example output (abbreviated, showing error):

    ```yaml
    status:
      conditions:
      - status: "False"
        type: Succeeded
        reason: TaskNotFound
      taskRuns:
        task-run-name:
          status:
            conditions:
            - message: 'Task "my-task" not found'
    ```

2. **Check Task References**:

    ```bash
    cat .tekton/pipelinerun.yaml | grep -A 5 taskRef
    ```

Example output:

    ```yaml
    taskRef:
      name: my-task
      kind: Task
    ```

3. **Verify Task Exists**:

    ```bash
    # For local tasks
    kubectl get task <task-name> -n <namespace>
    ```

Example output (if task exists):

    ```
    NAME       AGE
    my-task    10m
    ```

If task doesn't exist:

    ```
    Error from server (NotFound): tasks.tekton.dev "my-task" not found
    ```

    ```bash
    # For DevOps Hub catalog tasks
    curl http://artifacthub-shim-api.artifacthub-shim-system.svc.cluster.local/api/v1/packages/tekton-task/catalog/<task-name>
    ```

Example output (if task exists):

    ```json
    {
      "id": 1,
      "name": "git-clone",
      "kind": "Task",
      "catalog": {
        "id": 1,
        "name": "Tekton"
      }
    }
    ```

4. **Check Network Connectivity**:

    ```bash
    # Test artifacthub-shim access
    curl http://artifacthub-shim-api.artifacthub-shim-system.svc.cluster.local/api/v1/packages/tekton-task/catalog/git-clone
    ```

Example output (if accessible):

    ```json
    {"id":1,"name":"git-clone",...}
    ```

    ```bash
    # Test remote URL access
    curl <remote-task-url>
    ```

Example output (if accessible):

    ```yaml
    apiVersion: tekton.dev/v1
    kind: Task
    ...
    ```

### Common Causes \{#common-causes-5}

- **Task name incorrect**: Typo in task name
- **Task not in namespace**: Task defined in different namespace
- **DevOps Hub unavailable**: Network issues accessing `artifacthub-shim`
- **Remote URL inaccessible**: Cannot fetch task from remote URL
- **Task resolver error**: Resolver configuration incorrect

### Solution \{#solution-5}

1. Verify task name spelling
2. Check task exists in correct namespace
3. Test `artifacthub-shim` connectivity
4. Verify remote URLs are accessible
5. Check task resolver configuration

## Variables Not Resolving \{#variables-not-resolving}

### Symptom \{#symptom-7}

Pipeline variables like `{{revision}}` are not being replaced.

### Troubleshooting Steps \{#troubleshooting-steps-7}

1. **Check Variable Syntax**:

    ```bash
    cat .tekton/pipelinerun.yaml | grep -E "\{\{|\}\}"
    ```

Example output:

    ```
    revision: {{revision}}
    repo_url: {{repo_url}}
    ```

2. **Validate YAML Syntax with Dry-Run**:

   Use `kubectl apply --dry-run=client` to validate the PipelineRun YAML syntax and catch errors early:

    ```bash
    kubectl apply --dry-run=client -f .tekton/pipelinerun.yaml -n <namespace>
    ```

Example output (if syntax is valid):

    ```
    pipelinerun.tekton.dev/my-pipeline created (dry run)
    ```

Example output (if there are syntax errors):

    ```
    error: error validating ".tekton/pipelinerun.yaml": error validating data: [ValidationError(PipelineRun.spec): unknown field "invalid-field" in tekton.dev.v1.PipelineRun.spec, ...]
    ```

**Note**: This dry-run check validates YAML syntax and Tekton schema, but it won't validate variable resolution (variables like `{{revision}}` will remain as literal strings in dry-run). For variable resolution issues, check PAC controller logs.

3. **Check PAC Controller Logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=100 | grep -i variable
    ```

Example output (example log entries):

    ```
    {"level":"warn","ts":"2024-01-01T12:00:00Z","logger":"controller","msg":"Unknown variable","variable":"{{Revision}}"}
    ```

4. **Verify Variable Names**:

   - `{{revision}}` - correct
   - `{{Revision}}` or `{{REVISION}}` - incorrect (case-sensitive)
   - Use double curly braces: `{{variable_name}}`

5. **Check PipelineRun Parameters**:

    First, get the PipelineRun name:

    ```bash
    kubectl get pipelinerun -n <namespace>
    ```

    Then check parameters (replace `<name>` with the actual PipelineRun name):

    ```bash
    kubectl get pipelinerun <name> -n <namespace> -o jsonpath='{.spec.params}'
    ```

Example output:

    ```json
    [
      {
        "name": "revision",
        "value": "abc1234"
      },
      {
        "name": "repo_url",
        "value": "https://gitlab.com/user/repo"
      }
    ]
    ```

### Common Causes \{#common-causes-6}

- **Syntax error**: Missing curly braces or incorrect format
- **Case sensitivity**: Variable names are case-sensitive
- **Variable not available**: Variable not available for current event type (e.g., `{{pull_request_number}}` is only available for pull_request events)
- **PAC version**: Older PAC version may not support certain variables

### Solution \{#solution-6}

1. Verify variable syntax: Use `{{variable_name}}` format, for example `{{revision}}` or `{{repo_url}}`
2. Check variable names are correct (case-sensitive): `{{repo_owner}}`, `{{source_branch}}`, `{{pull_request_number}}`
3. Ensure variable is available for event type: Some variables like `{{pull_request_number}}` are only available for `pull_request` events
4. Update PAC to latest version if needed

For a complete list of available variables, see [Parameterizing Commits and URLs](../how_to/maintain_pipeline_code.mdx#parameterizing-commits-and-urls).

### Verification \{#verification-4}

After applying solutions, verify the fix:

1. **Check if variables are resolved** in the created PipelineRun:

    ```bash
    kubectl get pipelinerun <name> -n <namespace> -o jsonpath='{.spec.params}' | jq .
    ```

    Variables should be replaced with actual values (e.g., `{{revision}}` should be replaced with a commit SHA).

2. **Check PAC Controller logs** for variable resolution:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=50 | grep -i variable
    ```

    There should be no warnings about unknown variables.

## Status Not Reported to GitLab \{#status-not-reported-to-gitlab}

### Symptom \{#symptom-8}

Pipelines run successfully but status is not shown in GitLab.

### Troubleshooting Steps \{#troubleshooting-steps-8}

1. **Check PAC Watcher Logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app.kubernetes.io/name=watcher --tail=100
    ```

Example output (example log entries):

    ```
    {"level":"error","ts":"2024-01-01T12:00:00Z","logger":"watcher","msg":"Failed to update status","error":"401 Unauthorized"}
    ```

2. **Check GitLab Token**:

    First, find the secret name from Repository CR:

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o jsonpath='{.spec.git_provider.secret.name}'
    ```

    Then check the secret (replace `<gitlab-secret>` with the actual secret name):

    ```bash
    kubectl get secret <gitlab-secret> -n <namespace> -o yaml
    ```

Example output (abbreviated, token is base64 encoded):

    ```yaml
    apiVersion: v1
    kind: Secret
    metadata:
      name: gitlab-secret
    data:
      token: Z2xwYXQt...
    ```

3. **Test GitLab API Access**:

    ```bash
    TOKEN=$(kubectl get secret <gitlab-secret> -n <namespace> -o jsonpath='{.data.token}' | base64 -d)
    curl --header "PRIVATE-TOKEN: $TOKEN" "https://gitlab.com/api/v4/user"
    ```

Example output (if token is valid):

    ```json
    {
      "id": 1,
      "username": "user",
      "email": "user@example.com"
    }
    ```

If token is invalid or expired:

    ```json
    {
      "message": "401 Unauthorized"
    }
    ```

4. **Check Repository CR**:

    ```bash
    kubectl get repository <repo-name> -n <namespace> -o yaml
    ```

Example output (abbreviated):

    ```yaml
    apiVersion: pipelinesascode.tekton.dev/v1alpha1
    kind: Repository
    metadata:
      name: my-repo
    spec:
      git_provider:
        type: gitlab
        secret:
          name: gitlab-secret
          key: token
    ```

5. **Verify PipelineRun Status**:

    List PipelineRuns to find the name:

    ```bash
    kubectl get pipelinerun -n <namespace>
    ```

    Then check status (replace `<name>` with the actual PipelineRun name):

    ```bash
    kubectl get pipelinerun <name> -n <namespace>
    ```

Example output:

    ```
    NAME                    STARTED        DURATION   STATUS
    simple-pipeline-xxxxx   5 minutes ago  30s        Succeeded
    ```

### Common Causes \{#common-causes-7}

- **GitLab token invalid**: Token expired or incorrect
- **Token permissions**: Token lacks required scopes
- **Network issues**: Cannot reach GitLab API
- **PAC Watcher not running**: Watcher pod not running
- **Repository CR misconfigured**: Incorrect GitLab configuration

### Solution \{#solution-7}

1. Verify GitLab token is valid and not expired
2. Ensure token has `api` scope
3. Test GitLab API connectivity
4. Check PAC Watcher pod is running
5. Verify Repository CR GitLab configuration

### Verification \{#verification-5}

After applying solutions, verify the fix:

1. **Check PAC Watcher logs** for successful status updates:

    ```bash
    kubectl logs -n <pac-namespace> -l app.kubernetes.io/name=watcher --tail=50 | grep -i status
    ```

    You should see log entries indicating status updates were sent successfully.

2. **Check GitLab UI**:
   - Go to your GitLab project
   - Check the Merge Request or commit
   - Verify that pipeline status is displayed (e.g., "passed", "failed", "running")

3. **Verify PipelineRun completed**:

    ```bash
    kubectl get pipelinerun -n <namespace> --sort-by=.metadata.creationTimestamp | tail -1
    ```

    The PipelineRun should show `Succeeded` or `Failed` status, and this status should be reflected in GitLab.

## Comment Commands Not Working \{#comment-commands-not-working}

### Symptom \{#symptom-9}

Comment commands like `/retest` are not triggering pipelines.

### Troubleshooting Steps \{#troubleshooting-steps-9}

1. **Check Annotation Configuration**:

    ```bash
    cat .tekton/pipelinerun.yaml | grep on-comment
    ```

    Example output:

    ```
    pipelinesascode.tekton.dev/on-comment: "retest"
    ```

2. **Verify Comment Format**:

   - Commands must be on their own line or at the start of a line
   - `/retest` - correct
   - `Please /retest` - incorrect (command not at start)

3. **Check PAC Controller Logs**:

    ```bash
    kubectl logs -n <pac-namespace> -l app=pipelines-as-code-controller --tail=100 | grep -i comment
    ```

    Example output (example log entries):

    ```
    {"level":"info","ts":"2024-01-01T12:00:00Z","logger":"controller","msg":"Processing comment command","command":"/retest"}
    {"level":"warn","ts":"2024-01-01T12:00:01Z","logger":"controller","msg":"Comment command not at start of line","comment":"Please /retest"}
    ```

4. **Verify Merge Request Exists**:

   - Comment commands only work in Merge Requests
   - Not available for regular commits

5. **Check Webhook Configuration**:

    ```bash
    # In GitLab, verify webhook includes "Comments" trigger
    ```

### Common Causes \{#common-causes-8}

- **Annotation missing**: `on-comment` annotation not configured
- **Command format**: Command not at start of line
- **Not in Merge Request**: Comment commands only work in Merge Requests
- **Webhook not configured**: Comments event not enabled in webhook
- **Command name mismatch**: Command in comment doesn't match annotation

### Solution \{#solution-8}

1. Add `on-comment` annotation: `pipelinesascode.tekton.dev/on-comment: "retest"`
2. Ensure commands are at the start of a line
3. Use commands only in Merge Requests
4. Enable "Comments" in GitLab webhook configuration
5. Match command names exactly (case-sensitive)

## PAC Setting Has No Effect \{#pac-setting-has-no-effect}

### Symptom \{#symptom-10}

A setting was added to `TektonConfig.spec.platforms.kubernetes.pipelinesAsCode.settings` and applied without error, but PAC keeps behaving as if the setting still had its default value. Both `TektonConfig` and the derived `OpenShiftPipelinesAsCode` CR report ready, no event or error is recorded, and reading `TektonConfig` back shows the key is gone from `settings`.

This affects the Alauda extension settings only: `custom-console-url-namespace-vars`, `replace-empty-template-vars-with-empty` and `enable-pull-request-number-on-push-events`.

Note that the symptom is "the default is in effect", not "the feature is missing". When the key is absent from its ConfigMap, PAC falls back to the built-in default for that setting, so turning a default-on setting off is just as affected as turning a default-off setting on.

### Troubleshooting Steps \{#troubleshooting-steps-10}

The commands below use two placeholders. Set them first, in the same shell, so the rest can be pasted as is:

    ```bash
    SETTING=custom-console-url-namespace-vars   # the setting you configured
    PAC_NS=tekton-pipelines                     # spec.targetNamespace of TektonConfig
    ```

1. **Check whether the key survived in `TektonConfig`**:

    ```bash
    kubectl get tektonconfig config \
      -o jsonpath="{.spec.platforms.kubernetes.pipelinesAsCode.settings.$SETTING}"
    ```

    This step only tells you something if you wrote the key under `settings`; empty output then means the operator dropped it. If you already moved the key to `options.configMaps`, empty output here is the expected result — go straight to step 3.

2. **Check the derived CR**. On `profile: all`, which is what this symptom implies since PAC is running, the operator copies `settings` from `TektonConfig` into it, so the key is gone here too — this step only confirms the copy is in sync, it is not the authoritative check:

    ```bash
    kubectl get openshiftpipelinesascodes.operator.tekton.dev pipelines-as-code \
      -o jsonpath="{.spec.settings.$SETTING}"
    ```

3. **Check the ConfigMap PAC actually reads**. This is the authoritative check — `settings` only describes intent:

    ```bash
    kubectl get configmap pipelines-as-code -n "$PAC_NS" \
      -o jsonpath="{.data.$SETTING}"
    ```

    Compare the output with the value you intended. Anything else means PAC is not using your configuration. Note that empty output is the correct result when you deliberately set the key to an empty string, for example to clear a mapping you no longer want.

### Common Causes \{#common-causes-9}

- **Key written under `settings`**: the operator's defaulting webhook rebuilds that map from the upstream Pipelines as Code settings struct. The Alauda extension settings have no field in that struct, so they are discarded on save — silently, with no error and no event. This happens on `TektonConfig` itself, so the key never reaches the derived CR either.
- **Key written on the derived `OpenShiftPipelinesAsCode` CR**: on `profile: all` the operator copies `settings` and `options` back over from `TektonConfig` on the next reconcile, so the edit is lost even for keys it would otherwise accept.
- **Key written directly into the ConfigMap**: a manual edit survives ordinary reconciles, because the operator compares its rendered manifest against a hash annotation that your edit does not change. The comparison is per resource, so a PAC change that does not alter the rendered ConfigMap — a `spec.config` or Deployment option, for example — leaves the edit in place. It is silently overwritten the moment the rendered ConfigMap itself changes: any edit to `settings` or to `options.configMaps`, an operator upgrade, or a rebuild of the installer set. Editing the ConfigMap by hand therefore looks like it worked and then stops working at an unpredictable moment.

### Solution \{#solution-9}

Move the key to `options.configMaps` in `TektonConfig`, which the operator merges into the ConfigMap key by key and never rewrites:

    ```yaml
    spec:
      platforms:
        kubernetes:
          pipelinesAsCode:
            enable: true
            options:
              disabled: false
              configMaps:
                pipelines-as-code:
                  data:
                    custom-console-url-namespace-vars: "project=label:cpaas.io/project,cluster=label:cpaas.io/cluster"
    ```

Then repeat step 3 to confirm the value reached the ConfigMap. Details and the full key list are in [Where to Put Each Setting](../install/manage_pac.mdx#where-to-put-each-setting).

## Getting Help \{#getting-help}

If you're still experiencing issues:

1. **Review Logs**: Check all component logs for errors
2. **Verify Configuration**: Double-check all configuration files
3. **Check Documentation**: Review the PAC documentation in this guide
4. **Review Troubleshooting**: Check other troubleshooting sections in this document

## Next Steps \{#next-steps}

- [Manage PAC Component](../install/manage_pac.mdx) - Deployment and management guide
- [Guides](../guides/index.mdx) - Repository setup walkthroughs
- [Trigger PAC Pipelines](../how_to/trigger_pipeline.mdx) - Automatic events and comment commands
- [Configure Custom Certificates](../install/configure_custom_certificates.mdx) - Trust self-signed or private-CA Git providers
- [Configure Enhancement Features via TektonConfig](../../configure/configure-tektoncd-enhancement-configmap.mdx) - Automatic webhook registration settings
