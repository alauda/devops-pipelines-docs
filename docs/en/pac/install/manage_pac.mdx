...
If `artifacthub-shim` is deployed in a different namespace or you want to use an external Hub:

    ```yaml
    spec:
      settings:
        # For a cluster-internal artifacthub-shim in another namespace
        hub-url: "http://artifacthub-shim-api.devops-system.svc.cluster.local"
        hub-catalog-type: "artifacthub"
        hub-catalog-name: "catalog"
