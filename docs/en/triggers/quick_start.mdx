...
3. **Trigger does not respond**
   ...
   - Verify the shared `Ingress` is correctly configured (it is created in the Tekton system namespace and named after the export rule, not per EventListener): `kubectl get ingress -n tekton-pipelines`
