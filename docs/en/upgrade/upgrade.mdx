...
- `stable` (default from `v4.2.5`): Tracks `v4.2.z` LTS patch releases. Recommended for production.
- `latest`: Tracks every `v4.2.z` release. Equivalent to `stable` on the 4.2 LTS line, and was the default prior to `v4.2.5`.
- `pipelines-4.2`: Tracks `v4.2.z` patch updates only — same content as `stable`.

:::note
All three channels carry the same `v4.2.z` patch stream on the release-4.2 line. The channel choice mostly affects which channel name appears in your Subscription; upgrade behavior is the same across all three.
:::
