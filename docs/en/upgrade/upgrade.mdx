...
:::warning
Starting from `v4.12.0`, the built-in `Tekton Hub` is no longer deployed by default. When you upgrade to `v4.12.0` or later, an existing built-in `Tekton Hub` deployment is **removed** by the operator. Migrate catalog publishing and consumption to the standalone `ArtifactHub` component before upgrading whenever possible. For details, see <ExternalSiteLink name="artifacthub-shim" href="/catalog/index.html" children="ArtifactHub Shim Catalog" />.
:::
