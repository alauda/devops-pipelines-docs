...
:::warning
Starting from `v4.12.0`, the built-in `Tekton Hub` is no longer deployed by default. When you upgrade to `v4.12.0` or later, an existing built-in `Tekton Hub` deployment is **removed** by the operator. If you still rely on the built-in `Tekton Hub`, **record the catalog image before the upgrade**, then re-enable the hub and pin the catalog to that image after the upgrade. For details, see [Enabling the Built-in Tekton Hub](../hub/configure/enabling-tekton-hub.mdx).
:::
...
- `pipelines-4.10`: Tracks patch updates in the `v4.10` LTS line only.
...
2. **Switch to Target Channel**
   - If you need to upgrade to a version in a different channel, patch the subscription:
     ...
     ```
     Replace `<target-channel>` with the desired channel (e.g., `stable`, `latest`, `pipelines-4.10`).
