...
:::note
Starting from `v4.0.18`, fresh installations no longer deploy the in-cluster `Tekton Hub` by default. When upgrading from an earlier `v4.0.x` release that already has `Tekton Hub` deployed, the existing Hub and its catalog artifacts are preserved so existing Task and Pipeline references continue to resolve.

The `AUTOINSTALL_TEKTONHUB` setting supports `preserve` (default), `true`, and `false`. Do not set it to `false` during a routine patch upgrade because that value explicitly removes an existing Hub.

For fresh installations or a planned migration from the built-in Hub, see [Configuring Tekton to Use ArtifactHub Shim](./hub/configure/configuring-artifacthub-shim.mdx).
:::
...
- `stable` (default from `v4.0.17`): Tracks `v4.0.z` LTS patch releases. Recommended for production.
- `latest`: Tracks every `v4.0.z` release. Equivalent to `stable` on the 4.0 LTS line, and was the default prior to `v4.0.17`.
- `pipelines-4.0`: Tracks `v4.0.z` patch updates only - same content as `stable`.

:::note
All three channels carry the same `v4.0.z` patch stream on the release-4.0 line. The channel choice mostly affects which channel name appears in your Subscription; upgrade behavior is the same across all three.
:::
...
2. **Switch to Target Channel**
   - If you need to upgrade to a version in a different channel, patch the subscription:
     ...
     ```
     Replace `<target-channel>` with the desired channel (for example, `stable`, `latest`, or `pipelines-4.0`).
...
### Verify Preserved Tekton Hub Resources

If the cluster had an in-cluster `Tekton Hub` before upgrading to `v4.0.18`, perform these additional checks:

1. Confirm that the `TektonHub` resource is present and ready:
   ```bash
   kubectl get tektonhub -A
   ```
2. Confirm that the Hub API and catalog sidecar are running:
   ```bash
   kubectl get pods -A | grep tekton-hub-api
   ```
   Confirm that the `READY` count for the `tekton-hub-api` pod is complete.
3. Run an existing pipeline that uses `resolver: hub` and verify that its `PipelineRun` succeeds.

For a fresh `v4.0.18` installation, the absence of an in-cluster `Tekton Hub` is expected with the default settings. Configure and verify catalog resolution through the [ArtifactHub Shim](./hub/configure/configuring-artifacthub-shim.mdx) instead.
