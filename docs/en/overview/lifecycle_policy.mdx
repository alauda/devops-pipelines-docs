...
| v4.1.x | 2025-07-16 | 2025-11-16 | Non-LTS | `latest` | `latest` |
| v4.0.x | 2025-04-08 | 2026-04-12 | LTS | `stable`, `latest`, `pipelines-4.0` | `stable` |
...
The `Alauda DevOps Pipelines` operator on the `v4.2.z` LTS line publishes the following subscription channels:

- `stable` (default from `v4.2.5` onward): Receives `v4.2.z` LTS patch updates only. Recommended for production.
- `latest`: Receives every published `v4.2.z` release. This was the default channel prior to `v4.2.5`; keep your Subscription on `latest` if you want to preserve the previous behavior.
- `pipelines-4.2`: Equivalent to `stable` on this release line — pinned to `v4.2.z` patch updates.

:::note
Prior to `v4.2.5`, the `v4.2.z` bundle was published with `latest` as the default channel. Existing subscriptions on `latest` continue to work; no action is required unless you want to switch to `stable`.
:::
