...
- Tekton Hub Integration
  ...
  Access to a catalog of reusable Tasks and Pipelines to accelerate development. Starting from `v4.14.0`, the built-in `Tekton Hub` is no longer shipped; catalog resources are delivered by the standalone `ArtifactHub` component.
