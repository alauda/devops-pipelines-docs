...
:::note
Starting with `v4.2.5`, the default subscription channel for this bundle changes from `latest` to `stable`. Both channels carry the same `v4.2.z` patch stream on the 4.2 LTS line, so existing Subscriptions on `channel: latest` continue to work with no action required. To opt out of the new default explicitly, keep `channel: latest` in your Subscription YAML.
:::
...
| :------ | :------- | :------ |
| `stable` | `v4.2.z` LTS patch releases | Yes, from `v4.2.5` |
| `latest` | Every `v4.2.z` release | Previous default (prior to `v4.2.5`) |
| `pipelines-4.2` | `v4.2.z` LTS patch releases | No |
...
- Subscription channel layout aligned with the LTS channel policy. The bundle is now published to `stable`, `latest`, and `pipelines-4.2`, with `stable` as the new default. Existing Subscriptions on `latest` are not impacted; explicitly keep `channel: latest` if you want to pin to the previous default.

### Fixed Issues

{/* release-notes-for-bugs?template=fixed&project=DEVOPS&version=v4.2.5 */}

### Known Issues

{/* release-notes-for-bugs?template=unfixed&project=DEVOPS&version=v4.2.5 */}
