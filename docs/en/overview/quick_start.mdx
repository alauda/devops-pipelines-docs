...
Starting from `v4.12.0`, the built-in `Tekton Hub` is not installed by default; to deploy it as a transitional option, see [Enabling the Built-in Tekton Hub](../hub/configure/enabling-tekton-hub.mdx).
