...
Starting from `v4.14.0`, the built-in `Tekton Hub` is no longer shipped. Catalog resources are delivered by the standalone `ArtifactHub` component; see <ExternalSiteLink name="artifacthub-shim" href="/catalog/index.html" children="ArtifactHub Shim Catalog" />.
