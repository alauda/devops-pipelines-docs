...
This guide uses the default `artifacthub-shim` Service and namespace. If you want to keep or re-enable the built-in `Tekton Hub` instead, see [`Tekton Hub` Configuration](./tekton-hub-config.mdx).
...
If you are replacing an existing built-in `Tekton Hub`, do not disable it yet. First follow [Migrate Custom Catalogs from the Built-in Tekton Hub](#migrate-custom-catalogs), verify the migrated sources, and configure `TektonConfig` before removing the old Hub deployment.

## Migrate Custom Catalogs from the Built-in Tekton Hub \{#migrate-custom-catalogs}

This release preserves an existing built-in `Tekton Hub`, but it does not automatically migrate custom catalog configuration to `artifacthub-shim`. Complete the following steps manually before disabling the built-in Hub.
