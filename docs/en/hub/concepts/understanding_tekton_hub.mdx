...
:::warning
Starting from `v4.12.0`, the built-in `Tekton Hub` is no longer deployed by default — Tekton catalog artifacts are delivered by the platform's standalone `ArtifactHub` component instead. The built-in `Tekton Hub` remains available as a transitional option; see [Enabling the Built-in Tekton Hub](../configure/enabling-tekton-hub.mdx).
:::
...
### Resource Management Workflow \{#resource-management-workflow}
